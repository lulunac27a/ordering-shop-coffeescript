...
updateList = ->
  ...
  orders.forEach (order, i) ->
    ...
    html += '<td style="text-align: right;">$' + order.price.toLocaleString('en-US', {
      minimumFractionDigits: 2,
      maximumFractionDigits: 2
    }) + '</td>'
    html += '<td style="text-align: right;">' + order.tax.toLocaleString('en-US', {
      minimumFractionDigits: 2,
      maximumFractionDigits: 2
    }) + '%</td>'
    html += '<td style="text-align: right;">$' + (order.quantity * order.price * (1 + order.tax / 100)).toLocaleString('en-US', {
      minimumFractionDigits: 2,
      maximumFractionDigits: 2
    }) + '</td>'
    html += '<td><button onclick="deleteOrder(' + i + ')">Delete</button></td></tr>'
  html += '<tr><td colspan="4"></td><td style="text-align: right;">Total: $' + total.toLocaleString('en-US', {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2
  }) + '</td><td></td></tr>'
  html += '</table>'
